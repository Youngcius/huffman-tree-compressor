...
unsigned char
BitQueue(LinkQueue &Q) { // 取八位01010101 "字符" 变成一个unsigned char
    unsigned char ch1;
    unsigned char ch = 0;
...
void SelectMin(HuffmanTree HT, int n, int &s1,
               int &s2) { // HT[s1].weight < HT[s2].weught
...
void Huffmancoding(HuffmanTree &HT, Huffmancode &HC, int *weights,
                   int n) { // weights：0----n-1
    if (n <= 1)
        return;
    HuffmanTree p;
    ...
    if (!HT)
        exit(10);
    HT[0].weight = HT[0].lchild = HT[0].rchild = HT[0].parent = 0;
    for (i = 1; i <= n; i++) {
    ...
    char *code = (char *)malloc(
        n * sizeof(char)); // 非叶子节点n-1个，故而编码长度最大n-1
    code[n - 1] = '\0';
    int start, c, f;
    for (i = 1; i <= n; i++) {
        start = n - 1;
...
void Decompress(char *ZipName) {
    ...
    int mm = 2 * MAX - 1;
    int i, j;
    HT = (HuffmanTree)malloc((mm + 1) * sizeof(HTNode));
    if (!HT)
        exit(10);
    ...
    char *Code;
    unsigned char ch;
    if (!(Code = (char *)malloc(8 * sizeof(char))))
        exit(10);
...
int main(int argc, char *argv[]) {
    /*
            ./compress
            ./compress -c <file_to_compress>
            ./compress -d <file_to_decompress>
    */
    char FileName[NAMESIZE], ZipName[NAMESIZE], DeZipName[NAMESIZE];
    if (argc == 1) {
        cout << "Interactive compress/decompress mode" << endl;
        int choice;
        ...
        switch (choice) {
        case 1:
            cout << "Please input the name of the file you want to compress: ";
            scanf("%s", FileName);
            Compress(FileName);
            cout << "The compression has been finished." << endl;
            break;
        case 2:
            cout << "Please input the name of the file you want to decompress: ";
            scanf("%s", ZipName);
            Decompress(ZipName);
